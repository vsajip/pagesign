...
the functionality provided by `age <https://age-encryption.org/>` and `minisign
...
The `Identity` class represents an identity, which can either be a local identity
...
These are stored in attributes of an `Identity` instance named `created`,
...
so via `this link <https://github.com/vsajip/pagesign/issues/new/choose>`_.
